...
class XenForo_Helper_Facebook
{
	...
	public static function getUserInfo($accessToken, $path = 'me')
	{
		try
		{
			$client = XenForo_Helper_Http::getClient('https://graph.facebook.com/v2.4/' . $path);
			$client->setParameterGet('fields', 'id,name,email,birthday,gender,website,location');
			if ($accessToken)
			{
	...
	public static function getFacebookRequestUrl($redirectUri, $appId = null, $state = null)
	{
		...
		return 'https://www.facebook.com/v2.4/dialog/oauth?client_id=' . $appId
			. '&scope=' . $perms
			. '&state=' . $state

...
class XenForo_Model_Thread extends XenForo_Model
{
	...
	/**
	 * Determines if the user can edit tags on the thread with the given permissions.
	 * This does not check thread viewing permissions.
	 *
	 * @param array|null $thread Info about the thread (null when creating a thread)
	 * @param array $forum Info about the forum the thread is in
	 * @param string $errorPhraseKey Returned phrase key for a specific error
	 * @param array|null $nodePermissions
	 * @param array|null $viewingUser
	 ...
	 * @return boolean
	 */
	public function canEditTags(array $thread = null, array $forum, &$errorPhraseKey = '', array $nodePermissions = null, array $viewingUser = null)
	{
		...
		if (!$thread || $thread['user_id'] == $viewingUser['user_id'])
		{
			if (XenForo_Permission::hasContentPermission($nodePermissions, 'tagOwnThread'))
			{
				return true;
			}
		}
		...
		if (
			XenForo_Permission::hasContentPermission($nodePermissions, 'tagAnyThread')
			|| XenForo_Permission::hasContentPermission($nodePermissions, 'manageAnyTag')
		)
		{
			return true;
		}
	...
	public function prepareThread(array $thread, array $forum, array $nodePermissions = null, array $viewingUser = null)
	{
		...
		$thread['lastPostInfo'] = array(
			...
			'isIgnoring' => XenForo_Visitor::getInstance()->isIgnoring($thread['last_post_user_id'])
		);
		if (isset($thread['last_post_gender']))
		{
			$thread['lastPostInfo']['gender'] = $thread['last_post_gender'];
			$thread['lastPostInfo']['avatar_date'] = $thread['last_post_avatar_date'];
			$thread['lastPostInfo']['gravatar'] = $thread['last_post_gravatar'];
		}

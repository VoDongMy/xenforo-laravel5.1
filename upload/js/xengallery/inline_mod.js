	...
	XenForo.InlineModForm.prototype =
	{
		__construct: function($form)
		{
			...
			var $controls = $(this.$form.data('controls')),
				...
				countContainerClass = this.$form.data('selcountcont'),
				$countContainer = $(countContainerClass),
				selectionCountClass = this.$form.data('selcount'),
				$selectionCount = $controls.find(selectionCountClass).click($.context(this, 'nonCheckboxShowOverlay')),
				id = $controls.attr('id');

			this.$selectionCount = selectionCountClass;
			this.$id = id + 'Wrapper';
			...
			this.$checkAll = this.$form.find('#' + id + 'ModerationCheck').click($.context(this, 'checkAll'));
			$controls.show().appendTo('body').wrap('<form id="' + this.$id + '" class="InlineModOverlay" />');

			if ($countContainer.length)
			{
				$controls.find(this.$form.data('selcount'))
					.clone(true)
			...
			this.$totals = $(selectionCountClass + ' .InlineModCheckedTotal');
		...
		setState: function()
		{
			...
			this.$totals.closest(this.$selectionCount + '.cloned')[(total ? 'addClass' : 'removeClass')]('itemsChecked');
		...
		nonCheckboxShowOverlay: function(e)
		{
			...
			if ($(e.target).parents('#' + this.$id).length)
			{
				return;
			}

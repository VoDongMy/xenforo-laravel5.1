...
/**
 * Data writer for custom gallery fields.
 */
class XenGallery_DataWriter_Field extends XenForo_DataWriter
{
	/**
	...
	protected function _getFields()
	{
		return array(
			'xengallery_field' => array(
				...
				'display_group'         => array('type' => self::TYPE_STRING, 'default' => 'below_media',
					'allowedValues' => array('below_media', 'below_info', 'extra_tab', 'new_tab')
				),
				...
				'album_use'				=> array('type' => self::TYPE_UINT, 'default' => 1),
				'display_add_media'		=> array('type' => self::TYPE_BOOLEAN, 'default' => 0),
	...
	protected function _getExistingData($data)
	{
		...
		return array('xengallery_field' => $this->_getFieldModel()->getGalleryFieldById($id));
	}
	...
	protected function _verifyFieldId(&$id)
	{
		...
		if ($id !== $this->getExisting('field_id') && $this->_getFieldModel()->getGalleryFieldById($id))
		{
	...
	protected function _preSave()
	{
		...
		{
			$typeMap = $this->_getFieldModel()->getGalleryFieldTypeMap();
	...
	protected function _postSave()
	{
		...
		$this->_rebuildGalleryFieldCache();
	}
	...
	protected function _updateCategoryAssociations(array $categoryIds)
	{
		...
		$db->delete('xengallery_field_category', 'field_id = ' . $db->quote($fieldId));
	...
	{
		$fieldId = $this->get('field_id');
		$updateCategoryIds = $this->_getFieldModel()->getCategoryAssociationsByField($fieldId);

		$this->_deleteMasterPhrase($this->_getTitlePhraseName($fieldId));
		$this->_deleteMasterPhrase($this->_getDescriptionPhraseName($fieldId));
		$this->_deleteExistingChoicePhrases();

		$this->_db->delete('xengallery_field_value', 'field_id = ' . $this->_db->quote($fieldId));
		$this->_db->delete('xengallery_field_category', 'field_id = ' . $this->_db->quote($fieldId));
	...
	{
		return $this->_getFieldModel()->getGalleryFieldTitlePhraseName($id);
	}
	...
	{
		return $this->_getFieldModel()->getGalleryFieldDescriptionPhraseName($id);
	}
	...
	{
		return $this->_getFieldModel()->getGalleryFieldChoicePhraseName($fieldId, $choice);
	}

	protected function _rebuildGalleryFieldCache()
	{
		return $this->_getFieldModel()->rebuildGalleryFieldCache();
	}
